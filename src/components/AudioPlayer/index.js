...
const AudioPlayer = props => {
  ...
  React.useEffect(() => {
    if (
      ...
      props.currentMessage._id === previousId.current
    ) {
      player.stopPlayer();
  ...
  const onStatusPress = React.useCallback(
    e => {
  ...
  const playWidth = React.useMemo(
    () =>
      (state.currentPositionSec / state.currentDurationSec) * PLAY_WIDTH || 0,
    [state],
  );
  ...
  return (
    <Wrapper>
      ...
      <Container active={props.position === 'right'}>
        <Row>
          <Btn onPress={() => playPause(props.currentMessage._id)}>
            <Icon
              name={
              ...
              size={28}
              color={props.position === 'right' ? '#fff' : '#505050'}
            />
          </Btn>
          <StatusContainer onPress={onStatusPress}>
            <ViewBar white={props.position === 'right'}>
              <ViewBarPlay
                ...
                white={props.position === 'right'}
              />
            </ViewBar>
          </StatusContainer>
        </Row>
        <DurationTxt white={props.position === 'right'}>
