...
const ChatInput = ({value, onChange, onSend, appendMessage, ...props}) => {
  ...
  const uploadFile = React.useCallback(async (file, obj = true) => {
    ...
    const res = await (
      await fetch(constants.base_url + '/api/v1/chat/file', data)
    ).json();
    return res.path;
  }, []);
  ...
  return (
    <>
      <InputContainer>
        <Input value={value} onChangeText={onChange} placeholder={'Message'} />
        {onRec ? (
        ...
        <IconContainer onPress={onRec ? stopRecord : submit}>
          <Icon
            ...
            color={onRec ? 'green' : theme.primary}
          />
        </IconContainer>
      </InputContainer>
      <LocationModal
        visible={locModalVisible}
        close={() => setLocModalVisible(false)}
        onSend={sendLocation}
      />
    </>
  );
